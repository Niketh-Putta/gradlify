# Partner reply library
**Send from:** niketh13putta@gmail.com (or team@gradlify.com if thread is there)  
**Attach when noted:** `outreach/assets/Gradlify-Partner-Overview.pptx`  
**Rule:** Reply in same thread. Sutton 11 Plus = do not contact.

---

## Voice-first rule (read before every send)

**Goal of every email = book a 10-minute call.** Do not negotiate terms over email.

**Default reply when they ask for details/deck:**
```
Hi [Name],

Happy to send the overview - easiest if we do a quick 10-min call so I can show you the platform and we can agree what fits your audience.

Are you free [TIME 1] or [TIME 2] this week? I'll send a Meet/Zoom link.

Best,
Niketh
```

**After every call:** email their `?ref=` link + caption within 1 hour.

**Call script:** see `docs/GRADLIFY-ACTION-SEQUENCE.md` → Block 2, Step 9.

---

## When to attach the deck

| Situation | Attach PPTX? |
|-----------|--------------|
| Asked for deck, slides, brief, overview, media kit | **Yes** |
| Asked for partner details / trial access | **Yes** + one-pager in body |
| Cold follow-up / "last check" | No |
| Polite close (declined commission) | No |

**Email framing for deck:**  
*"I've attached a short partner overview deck. It's the same product story we use with schools and tutors - happy to jump on a quick call to tailor it for your audience."*

---

## 1. Cecile - The Frenchie Mummy
**To:** cecile@thefrenchiemummy.com  
**Thread:** Re: 11+ paid/affiliate collab?  
**Needs:** Brief + deck + login fix  
**Attach:** Yes

```
Hi Cecile,

Thanks for your patience on this.

I've attached a short partner overview deck plus the key points below.

Partner summary:
- Gradlify = 11+ practice, mocks and resources for Year 4/5 families
- Your angle: extra practice between tutoring - not a replacement for tuition
- Offer: 30% of all payments through your referral link (code FRENCHIEMUMMY once we agree)

Access:
Please try signing up again at https://gradlify.com/11-plus using thefrenchiemummy@hotmail.com (or tell me which email you prefer and I'll activate creator access on my side).

If login still fails, send a screenshot of the exact error and I'll fix it today.

Once you've reviewed the platform, I can send your partner link and a short caption draft.

Best,
Niketh
gradlify.com
```

---

## 2. Chris - Eleven Plus Success
**To:** tutor@elevenplussuccess.com  
**Thread:** Re: 11+ affiliate partnership?  
**Needs:** Partner details + trial + deck  
**Attach:** Yes

```
Hi Chris,

Thanks - I've attached a short partner overview deck.

Partner structure:
- Eleven Plus Success shares Gradlify as extra 11+ practice between lessons
- 30% of all payments from families who sign up through your tracking link
- Gradlify: free practice + mocks to start; paid upgrade for unlimited mocks and progress tracking

To set up your trial access:
1. Create a free account at https://gradlify.com/11-plus
2. Reply with the email address you used
3. I'll activate full review access on my side

Once you've had a look, I'll send your partner link (https://gradlify.com/11-plus?ref=EPS) if the structure works for you.

Best,
Niketh
gradlify.com
```

---

## 3. Andy - Pinner Road Learning Centre
**To:** prlcharrow@gmail.com  
**Thread:** Re: 11+ affiliate partnership?  
**Needs:** Confirm 30% + deck for centre staff  
**Attach:** Yes

```
Hi Andy,

Following up - I've attached a short partner overview deck you can share with your team.

Quick recap:
- Pinner Road recommends Gradlify as extra timed practice between lessons
- 30% of all referral payments to Pinner Road
- Tracking link: https://gradlify.com/11-plus?ref=PRLC (live once you confirm the structure)

Does that work for Pinner Road Learning Centre? If yes, I'll confirm the link is active and send a one-line blurb for parent emails.

Happy to jump on a quick call if easier.

Best,
Niketh
gradlify.com
```

---

## 4. Mock Test Masters
**To:** info@mocktestmasters.com  
**Thread:** Re: affiliate partnership (or new if thread on team@)  
**Needs:** Call booking + deck  
**Attach:** Yes

```
Hi,

Hope you had a good week.

I've attached a short partner overview deck. At a high level: Gradlify gives families extra timed practice and mocks between formal mock-test sessions - we complement what you run, not replace it.

Are you free for a 10-minute call this week to agree referral terms? I'm flexible on times.

If easier by email:
- 30% of all payments through Mock Test Masters referrals
- I set up your tracking link after we agree
- Gradlify = between-lessons practice + progress tracking

What works for you?

Best,
Niketh
gradlify.com
```

---

## 5. David - 11 Plus Hub
**To:** info@11plushub.com  
**Thread:** Re: 11+ Paid Promo / partnership  
**Needs:** Outline before call - deck supports this  
**Attach:** Yes

```
Hi David,

As requested, here's a clearer outline - I've attached a short partner overview deck as well.

At a high level:
- Affiliate partnership (not a one-off ad fee)
- 11 Plus Hub families get Gradlify as extra timed practice between your lessons
- You earn on paid sign-ups through your tracked link
- Placement options: member email, resources page, or lesson notes - we decide on a short call

Exact commercial terms and placement are easiest on a 10-minute call rather than over email.

If you send a couple of times that work over the next few days, I'll confirm one.

Best,
Niketh
gradlify.com
```

---

## 6. Tina - MumFounded (CLOSED)
**Status:** Declined commission - polite close already sent. **No further action.**

---

## 7. Standard reply when they say YES (any partner)

```
Hi [Name],

Great - here's your partner link:

https://gradlify.com/11-plus?ref=[CODE]

Structure: 30% of all payments from families who sign up through your link.

Gradlify gives Year 4/5 families free practice and mocks; paid upgrades add unlimited mocks and deeper progress tracking. Position it as extra practice between lessons, not a replacement for tuition.

I've attached the partner overview deck if useful for your team. Shout if you want a caption draft or want to co-promote our 14 June live mock.

Best,
Niketh
```

---

## 8. Wave 1 - New creator outreach (July, after funnel fix)

**To:** hello@mathsaurus.com (and TD, Kin, Geek School, Robert Lomax)  
**Subject:** Quick question about a collab  
**Attach:** Only if they reply asking for overview

```
Hi [Name],

I'm Niketh, founder of Gradlify (gradlify.com). We help UK families with 11+ prep.

I've been following [specific channel/work] and think your audience is a strong fit.

We're setting up affiliate partnerships with a few 11+ educators - tracked link, 30% on referral payments, optional co-branded live mock.

Would you be open to a 10-minute call, or should I send a short partner overview first?

Best,
Niketh
gradlify.com
```

---

## Tracker update after each send

Update `outreach/manual-affiliate-tracker.csv`: `status`, `date_replied`, `notes`.
