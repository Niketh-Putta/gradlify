...
const formatAnswerText = (text: string) => String(text ?? '').trim();
...
export function TopicPracticeQuestions({ topicSlug, topicTitle, userId }: TopicPracticeQuestionsProps) {
  ...
  useEffect(() => {
    ...
    const loadTrack = async () => {
      let profileTrack: string | null = null;
      if (userId) {
        const { data } = await supabase
          ...
          .eq('user_id', userId)
          .maybeSingle();
        profileTrack = data?.track ?? null;
      }
      
      const resolved = resolveUserTrack(profileTrack);
      const forced = is11Plus ? '11plus' : (isGCSE ? 'gcse' : resolved);
      setUserTrack(forced);
    };

    void loadTrack();
  }, [userId, is11Plus, isGCSE]);
