          ...
          <div className="p-4 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-y-2 mb-4">
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                ...
                <span className="inline-flex items-center rounded-full border border-primary/20 bg-primary/10 px-1.5 py-0.5 text-[9px] sm:text-xs font-semibold text-primary">
                  {currentQuestion.marks ?? 1} mark{(currentQuestion.marks ?? 1) === 1 ? "" : "s"}
                </span>
                {currentDifficultyLevel && (
                  <span className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[9px] sm:text-xs font-medium ${getDifficultyTagClass(currentDifficultyLevel)}`}>
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                {subtopicLabel && (
                  <span className="inline-flex max-w-[120px] sm:max-w-[200px] items-center truncate rounded-md bg-muted px-1.5 py-0.5 text-[9px] sm:text-[11px] font-medium text-muted-foreground">
                    {subtopicLabel}
                  </span>
                )}
                {questionCalculatorLabel && questionCalculatorLabel !== 'Non-calc' && (
                  <span className="inline-flex items-center rounded-md bg-muted px-1.5 py-0.5 text-[9px] sm:text-[11px] font-medium text-muted-foreground">
                ...
                <button 
                  onClick={toggleFlag}
                  className={cn(
                    "flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2.5 py-1 sm:py-1.5 rounded-lg transition-all duration-200 hover:scale-105 border",
                    flagged.has(currentIndex) 
                      ? "border-amber-400/40 bg-amber-500/10 text-amber-500" 
                      : "border-transparent text-muted-foreground hover:bg-muted"
                  )}
                >
                  <Flag className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill={flagged.has(currentIndex) ? "currentColor" : "none"} />
                  <span className="text-[10px] sm:text-xs font-medium">{flagged.has(currentIndex) ? "Flagged" : "Flag"}</span>
                </button>
              </div>
            </div>

            {currentQuestion.prompt && (
              <div className="text-base sm:text-xl font-medium leading-relaxed mb-5 text-foreground">
