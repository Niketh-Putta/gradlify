...
export default function MathText({ text, display = false, className }: MathTextProps) {
  ...
  try {
    ...
    const mathRegex = /\\begin\{[a-zA-Z*]+\}[\s\S]*?\\end\{[a-zA-Z*]+\}|\\mathbf\{[^}]+\}|\\vec\{[^}]+\}|\\binom\{[^}]+\}\{[^}]+\}|\\text\{[^}]+\}|\\mathrm\{[^}]+\}|\\overline\{[^}]+\}|\\dot\{[^}]+\}|\([^)]*\)\^\{[^}]+\}|\([^)]*\)\^[0-9a-zA-Z]|\\(?:sin|cos|tan|sec|csc|cot)\([^)]+\)|\\(?:sin|cos|tan|sec|csc|cot|log|ln|pi|theta|alpha|beta|gamma|delta|sigma|omega|circ|neq|leq|geq|le|ge|equiv|mid|approx|cdots|gcd|lcm|pm|cdot|times|div|to|infty|sum|int|lim|dot|overline|Rightarrow|rightarrow|leftarrow|Leftarrow|leftrightarrow)|\\sqrt\\[[^\\]]+\\]\\{[^}]+\\}|\\[a-zA-Z]+\{[^}]+\}|\\[a-zA-Z]+|\b\d+(?![dD]\b)[A-Za-z]\b|\b[A-Za-z]\d+\b|[a-zA-Z0-9]+_\{[^}]+\}|[a-zA-Z0-9]+_[a-zA-Z0-9]+|[a-zA-Z0-9]+\^\{[^}]+\}|[a-zA-Z0-9]+\^\d+|[a-zA-Z0-9]+\^[a-zA-Z]|[a-zA-Z0-9]+\^\\circ|\b(?![AaIi]\b)[A-Za-z]\b|\b\d+(?:\.\d+)?\b|\d+°|√\d+|×|÷|·|⇒|→|←|⇐|↔|≤|≥|≠|≈|π/g;
