...
export function DiscordFooterEntry({
  ...
  if (variant === "spotlight") {
    return (
      <div
        className={clsx(
          "relative overflow-hidden rounded-[24px] border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0a0f0d] shadow-sm flex flex-col",
          className
        )}
      >
        {/* Subtle brand glow on the right */}
        <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-1/2 opacity-[0.08] dark:opacity-[0.15]"
          style={{ 
            background: "radial-gradient(circle at right center, #25d366 0%, transparent 70%)" 
          }} 
        />
        {/* Secondary ambient glow */}
        <div className="pointer-events-none absolute -bottom-20 -right-10 w-64 h-64 rounded-full opacity-10 dark:opacity-20 blur-3xl"
          style={{ background: "#25d366" }} 
        />

        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-8 p-6 sm:p-8">
            {/* Left: Content */}
            <div className="flex-1 min-w-0">
              ...
              {/* Title */}
              <h3 className="text-slate-900 dark:text-foreground font-bold text-xl sm:text-2xl leading-tight mb-2 tracking-tight">
                The Level Field
              </h3>

              {/* Description */}
              <p className="text-sm leading-relaxed mb-5 text-slate-500 dark:text-muted-foreground max-w-sm">
              ...
              <div className="flex items-center gap-4 mb-6">
                {[
                  ...
                  <div key={s.label} className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] font-medium text-slate-500 dark:text-muted-foreground">{s.label}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="hidden sm:flex shrink-0 w-64 h-48 relative justify-end items-center mr-4">
              ...
              <div className="relative w-56 h-40 bg-white/60 dark:bg-card/60 backdrop-blur-xl rounded-2xl border border-white dark:border-border shadow-xl shadow-emerald-900/5 flex flex-col justify-center items-center p-6 -rotate-2 transform overflow-hidden">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#25d366] to-[#128c47] flex items-center justify-center shadow-lg shadow-emerald-500/20 mb-3 relative z-10">
                  {/* WhatsApp SVG placeholder removed to fix esbuild parser bug */}
                  <div className="w-8 h-8 rounded-full border-2 border-white" />
                </div>
                <div className="h-1.5 w-16 bg-slate-200 dark:bg-slate-700/50 rounded-full mb-1.5 relative z-10" />
                <div className="h-1.5 w-10 bg-slate-100 dark:bg-slate-800/50 rounded-full relative z-10" />
              </div>
            </div>
          </div>
      </div>
