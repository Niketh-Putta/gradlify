          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
            <motion.div className="grid lg:grid-cols-12 gap-8 sm:gap-8 lg:gap-10 items-start" initial="hidden" animate="show" variants={motionCfg.stagger}>
              <motion.div variants={motionCfg.fadeUp} className="lg:col-span-5 lg:pr-4 flex flex-col justify-center">
                ...
                <p className={cn(
                  "mt-8 text-lg sm:text-xl font-medium max-w-lg leading-[1.6]",
              ...
              <motion.div variants={motionCfg.fadeUp} className="lg:col-span-7 relative mt-16 lg:mt-8 h-[460px] lg:h-[520px] w-full max-w-[600px] mx-auto lg:ml-auto">
                {/* 3D Stacked Deck - True Product Surface Overhaul */}
                
                {/* Card 1: TRUE Exam Readiness (Back Left) */}
                <div className={cn(
                  "absolute top-8 left-[-20px] w-[380px] rounded-[32px] border shadow-2xl transition-all duration-700 ease-out pointer-events-none hidden sm:block",
                  "transform origin-bottom-left -rotate-[12deg] scale-[0.9] -translate-x-16 -translate-y-12 opacity-100",
                  ...
                  <div className={cn("h-[280px] w-full p-0 flex flex-col rounded-[32px] overflow-hidden shadow-2xl transition-colors duration-700", isDark ? "bg-[#0B1528]" : "bg-white")}>
                    <img 
                ...
                <div className={cn(
                  "absolute top-12 right-[-20px] w-[380px] rounded-[32px] border shadow-2xl transition-all duration-700 ease-out pointer-events-none hidden sm:block",
                  "transform origin-bottom-right rotate-[14deg] scale-[0.9] translate-x-16 -translate-y-8 opacity-100",
                  ...
                  <div className={cn("h-[300px] w-full p-0 flex flex-col rounded-[32px] overflow-hidden shadow-2xl transition-colors duration-700", isDark ? "bg-[#0B1528]" : "bg-white")}>
                    <img 
                      src="/hero-spag.png" 
                ...
                <FloatingProductCard isDark={isDark} className="absolute bottom-[-10px] left-1/2 -translate-x-1/2 w-[95%] sm:w-[500px] z-30">

                  {/* Mock English Split-View Showcase */}
                  <div className="flex flex-col h-[340px] sm:h-[420px]">
                    {/* Header bar */}
                    <div className={cn("px-4 py-3 border-b flex items-center justify-between", isDark ? "bg-white/5 border-white/5" : "bg-slate-50/50 border-slate-100")}>
                      <div className="flex gap-1.5">
                        <div className="w-2.5 h-2.5 rounded-full bg-red-400/50" />
                        <div className="w-2.5 h-2.5 rounded-full bg-amber-400/50" />
                        <div className="w-2.5 h-2.5 rounded-full bg-emerald-400/50" />
                      </div>
                      <div className={cn("text-[10px] font-bold tracking-tight px-3 py-1 rounded-full uppercase", isDark ? "bg-white/10 text-white" : "bg-white text-slate-600 shadow-sm border border-slate-100")}>
                    ...
                    <div className="flex-1 flex overflow-hidden">
                      ...
                      <div className={cn("w-3/5 border-r p-6 py-5 select-none", isDark ? "border-white/5" : "border-slate-100")}>
                        <div className={cn("text-[9px] uppercase tracking-[0.25em] font-bold mb-6 opacity-40", isDark ? "text-white" : "text-slate-900")}>The Passage • Vol. 12</div>
                        <div className="space-y-4">
                           <div className={cn("font-serif text-sm leading-relaxed opacity-90", isDark ? "text-slate-100" : "text-slate-800")}>
                              The old oak tree stood like a silent sentinel over the sleeping village. Its gnarled branches reaching...
                           </div>
                           {/* Real-time Evidence Highlight */}
                           <div className={cn("relative p-3.5 rounded-xl border group/evidence transition-all duration-300", isDark ? "bg-amber-500/20 border-amber-500/40" : "bg-amber-50/70 border-amber-200/50 shadow-sm")}>
                              <div className={cn("font-serif text-sm leading-relaxed", isDark ? "text-amber-100" : "text-amber-950 font-medium")}>
                                 "The old oak tree stood like a <span className="underline decoration-amber-500/50 decoration-2 underline-offset-4">silent sentinel</span> over the sleeping village."
                              </div>
                              <div className="absolute -left-1.5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.6)] animate-pulse" />
                           </div>
                           <div className={cn("font-serif text-xs leading-relaxed opacity-40 italic", isDark ? "text-slate-100" : "text-slate-800")}>
                              Paragraph 2: The air was thick with the scent of damp earth and woodsmoke...
                           </div>
                        </div>
                      </div>

                      {/* Right Sidebar (Question) - Software Interface */}
                      <div className="flex-1 p-6 flex flex-col justify-center bg-black/5 dark:bg-white/[0.01]">
                         <div className={cn("mb-6 p-4 rounded-2xl border shadow-sm", isDark ? "bg-slate-900/60 border-white/5" : "bg-white border-slate-100")}>
                            <div className={cn("text-[10px] font-bold uppercase tracking-wider mb-2", isDark ? "text-amber-400" : "text-orange-600")}>Comprehension</div>
                            <div className={cn("text-[13px] font-semibold leading-snug", isDark ? "text-white" : "text-slate-900")}>
                               What literary device is used to describe the tree?
                            </div>
                         </div>
                         <div className="space-y-2.5">
                           {[
                             { label: "Onomatopoeia", id: "A" },
                             { label: "Simile", id: "B", active: true },
                             { label: "Metaphor", id: "C" },
                             { label: "Hyperbole", id: "D" }
                           ].map((opt) => (
                             <div key={opt.id} className={cn(
                               "w-full h-11 rounded-xl border flex items-center px-4 gap-3 transition-all duration-300", 
                               opt.active 
                                ...
                                <div className={cn("w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold", opt.active ? "bg-amber-500 text-white" : (isDark ? "bg-white/10 text-slate-400" : "bg-slate-100 text-slate-500"))}>
                                   {opt.id}
                                </div>
                                <div className={cn("text-xs font-semibold", opt.active ? (isDark ? "text-amber-100" : "text-amber-900") : (isDark ? "text-slate-400" : "text-slate-500"))}>
                                   {opt.label}
                                </div>
                                {opt.active && <Sparkles className={cn("w-3.5 h-3.5 ml-auto", isDark ? "text-amber-400" : "text-amber-500")} />}
                             </div>
                           ))}
                         </div>
                      </div>
