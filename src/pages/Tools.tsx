...
export default function Tools() {
  return (
    <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(1200px_460px_at_20%_-5%,rgba(239,68,68,0.18),transparent_62%),radial-gradient(900px_420px_at_82%_8%,rgba(251,146,60,0.2),transparent_68%),#fff7ed] text-slate-900 dark:bg-[radial-gradient(1200px_460px_at_20%_-5%,rgba(185,28,28,0.25),transparent_62%),radial-gradient(900px_420px_at_82%_8%,rgba(234,88,12,0.2),transparent_68%),#04050a] dark:text-white transition-colors duration-300">
      ...
      {/* Remove label to hide top strip, remove lockTheme to enable mode toggle */}
      <ToolsNav />
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-24 pt-16 sm:px-6 lg:px-8 relative z-10">
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          {[
            ...
            <div
              key={title}
              className="rounded-[24px] border border-slate-200/80 bg-white/60 dark:bg-black/20 dark:border-white/10 backdrop-blur-xl p-6 sm:p-8 shadow-xl dark:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.4)] group relative overflow-hidden transition-all duration-500 hover:border-orange-500/30 hover:shadow-[0_48px_80px_-16px_rgba(245,158,11,0.12)] opacity-85 hover:opacity-100"
            >
              ...
              <div className="relative z-10 flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#E65C2B] dark:text-amber-500">
                      Coming soon
                    </span>
                    <span
                      className="rounded-full border border-slate-200/80 dark:border-white/10 bg-white/60 dark:bg-white/10 px-2.5 py-0.5 text-[10px] font-bold text-slate-700 dark:text-white shadow-sm uppercase tracking-widest"
                    >
                      {track}
                    </span>
                  </div>
                  <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 tracking-tight leading-snug">{title}</h3>
                </div>
              </div>
              <p className="relative z-10 mt-3 text-sm font-medium leading-relaxed text-[#475569] dark:text-slate-400">{subtitle}</p>
              <button
                type="button"
                disabled
                className="relative z-10 mt-8 inline-flex w-full items-center justify-center rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-4 py-3 text-xs sm:text-sm font-bold tracking-wide text-slate-500 dark:text-slate-400 uppercase transition-colors"
              >
                Coming soon
              </button>
            </div>
        ...
        <p className="mt-12 text-sm font-medium text-slate-600 dark:text-slate-500 max-w-2xl">
