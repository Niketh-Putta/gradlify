...
async function searchLeaderboard() {
  console.log("Searching for 'varun' in 'month' leaderboard...");
  ...
  const matches = data.filter(entry => (entry.name || '').toLowerCase().includes('varun'));
  console.log('Matches in month leaderboard:', matches);

  console.log("Searching for 'varun' in 'all' leaderboard...");
  const { data: dataAll, error: errorAll } = await supabase.rpc('get_leaderboard_correct_global', {
    p_period: 'all'
  });

  if (!errorAll) {
    const matchesAll = dataAll.filter(entry => (entry.name || '').toLowerCase().includes('varun'));
