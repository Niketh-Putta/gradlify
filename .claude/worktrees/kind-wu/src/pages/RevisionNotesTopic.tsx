...
import { ArrowLeft, ArrowRight, Check, BookOpen, Clock, ChevronRight, CheckCircle, ChevronDown, Lightbulb, AlertTriangle, Target, FileText, Zap, HelpCircle, PenLine, Eye, EyeOff, FlaskConical, Compass } from "lucide-react";
...
const blockConfig: Record<string, { 
  ...
  methodology: {
    ...
    bgClass: "notes-block-methodology",
    borderClass: "border-sky-300/60 dark:border-sky-700/40",
    headerClass: "bg-sky-50/80 dark:bg-sky-950/30",
    priority: 'high'
  },
  pitfalls: {
    icon: <AlertTriangle className="h-5 w-5" />,
    color: "text-slate-600 dark:text-slate-300",
    bgClass: "notes-block-pitfalls",
    borderClass: "",
    headerClass: "bg-slate-100/60 dark:bg-slate-900/40",
    priority: 'high'
  },
  extension: {
    icon: <FlaskConical className="h-5 w-5" />,
    color: "text-indigo-600 dark:text-indigo-400",
    bgClass: "notes-block-extension",
    borderClass: "",
    headerClass: "bg-indigo-50/60 dark:bg-indigo-950/30",
    priority: 'high'
  },
  info: { 
...
function parseMarkdownToBlocks(md: string) {
  ...
  lines.forEach(line => {
    if (line.startsWith('## ')) {
      ...
      else if (title.includes('Methodology') || title.includes('Protocol') || title.includes('Standardised')) type = 'methodology';
      else if (title.includes('Analysis of Common') || title.includes('Pitfalls') || title.includes('Pitfall')) type = 'pitfalls';
      else if (title.includes('Extension') || title.includes('Scholarship')) type = 'extension';
      else if (title.includes('Worked Example') || title.includes('Worked Examination') || title.includes('Method') || title.includes('Exam-Style')) type = 'example';
      else if (title.includes('Common Mistakes') || title.includes('Critical Errors')) type = 'mistakes';
      else if (title.includes('Exam Tips') || title.includes('Exam Strategy')) type = 'tips';
      else if (title.includes('Summary')) type = 'summary';
      else if (title.includes('Formula')) type = 'formula';
      else if (title.includes('Quick Checks') || title.includes('Knowledge Check') || title.includes('Practice Questions')) type = 'checks';
...
export default function RevisionNotesTopic() {
  ...
  }, [topicSlug, contentBlocks.length]);
      ...
      <div className="max-w-[800px] mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="space-y-5 mb-10">
          {contentBlocks.map((block, index) => {
            ...
            const isPremiumType = block.type === 'methodology' || block.type === 'pitfalls' || block.type === 'extension';

            // Premium block type labels
            const premiumLabel: Record<string, string> = {
              methodology: 'Protocol',
              pitfalls: "Examiner's Analysis",
              extension: 'Scholar Extension',
            };
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  blockStyle.bgClass,
                  !isPremiumType && blockStyle.borderClass,
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      {isPremiumType && (
                        <span className={cn(
                          "inline-block text-[10px] font-bold uppercase tracking-widest mb-0.5 px-2 py-0.5 rounded",
                          block.type === 'methodology' && "bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300",
                          block.type === 'pitfalls' && "bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-300",
                          block.type === 'extension' && "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300",
                        )}>
                          {premiumLabel[block.type]}
                        </span>
                      )}
                      <h2 className={cn(
                        "font-semibold text-foreground block leading-snug",
