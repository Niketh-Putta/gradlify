...
// Grade helpers moved to `src/lib/readinessGrades.ts`

export function Readiness() {
  const { user, profile } = useAppContext();
  const navigate = useNavigate();
  const userTrack = resolveUserTrack(profile?.track ?? null);
  const applyTrackFilter = <T extends { eq: (column: string, value: string) => T; or: (condition: string) => T }>(builder: T): T => {
    if (userTrack === '11plus') {
    ...
    const run = async () => {
      ...
      try {
        ...
        const progressMap = new Map(
          (progress || []).map((p: any) => [`${p.topic_key}|||${p.subtopic_key}`, {
        ...
        const practiceStatsByTopic = (practiceData || []).reduce((acc: Record<string, { attempts: number; correct: number }>, row: any) => {
