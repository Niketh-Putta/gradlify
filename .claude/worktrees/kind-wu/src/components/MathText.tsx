...
function repairBrokenLatex(text: string): string {
  ...
  result = result.replace(/\s*\\(Rightarrow|rightarrow|Leftarrow|leftrightarrow)\s*/g, ' \\\\$1 ');
...
export default function MathText({ text, display = false, className }: MathTextProps) {
  ...
  try {
    ...
    const hasMathPatterns =
      ...
      || /\^\{|\^[0-9a-zA-Z]|_\{|_[0-9a-zA-Z]|×|÷|·|√|⇒|→|⇐|↔|≤|≥|≠|≈|π|\\begin\{|\\(sqrt|frac|overline|dot|binom|text|mathrm|mathbf|vec)\{|\\(times|div|cdot|pm|Rightarrow|rightarrow|Leftarrow|leftrightarrow|equiv|mid|approx|cdots|gcd|lcm)\b|\\[a-zA-Z]+/i.test(
        normalized,
    ...
    const mathRegex = /\\begin\{[a-zA-Z*]+\}[\s\S]*?\\end\{[a-zA-Z*]+\}|\\mathbf\{[^}]+\}|\\vec\{[^}]+\}|\\binom\{[^}]+\}\{[^}]+\}|\\text\{[^}]+\}|\\mathrm\{[^}]+\}|\\overline\{[^}]+\}|\\dot\{[^}]+\}|\([^)]*\)\^\{[^}]+\}|\([^)]*\)\^[0-9a-zA-Z]|\\(?:sin|cos|tan|sec|csc|cot)\([^)]+\)|\\(?:sin|cos|tan|sec|csc|cot|log|ln|pi|theta|alpha|beta|gamma|delta|sigma|omega|circ|neq|leq|geq|le|ge|equiv|mid|approx|cdots|gcd|lcm|pm|cdot|times|div|to|infty|sum|int|lim|dot|overline|Rightarrow|rightarrow|Leftarrow|leftrightarrow)|\\sqrt\\[[^\\]]+\\]\\{[^}]+\\}|\\[a-zA-Z]+\{[^}]+\}|\\[a-zA-Z]+|\b\d+\s*\([^)]*\)|\b\d+[A-Za-z]\b|\b[A-Za-z]\d+\b|[a-zA-Z0-9]+_\{[^}]+\}|[a-zA-Z0-9]+_[a-zA-Z0-9]+|[a-zA-Z0-9]+\^\{[^}]+\}|[a-zA-Z0-9]+\^\d+|[a-zA-Z0-9]+\^[a-zA-Z]|[a-zA-Z0-9]+\^\\circ|\b[A-Za-z]\b|\b\d+(?:\.\d+)?\b|\d+°|√\d+|×|÷|·|⇒|→|⇐|↔|≤|≥|≠|≈|π/g;
