-- Fix v_overall_readiness view to multiply by 2
-- The readiness values in readiness_history are stored at half scale
...
WITH canonical_topics AS (
  ...
  SELECT DISTINCT ON (rh.topic) 
    rh.user_id,
    rh.topic,
    rh.readiness_after * 2 AS readiness_after  -- Multiply by 2 to fix scale
