#!/usr/bin/env python3
"""Top up Number mini-subtopics to exactly 60 total questions each.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Top up Number subtopics to 60 total.")
    ...
    for subtopic_id in subtopics:
        ...
        if target_h > 0:
            ...
            added_h = target_h - remaining_h
        else:
            remaining_h = 0
            added_h = 0

        # Foundation up to target per tier.
        if target_f > 0:
            remaining_f = topup_tier(
                supabase_url,
                headers,
                subtopic_id,
                "Foundation Tier",
            ...
            added_f = target_f - remaining_f
        else:
            remaining_f = 0
            added_f = 0

        # If still below total target, top up Foundation with remainder.
        counts = fetch_counts(supabase_url, headers, subtopic_id)
        foundation = counts.get("Foundation Tier", 0)
        higher = counts.get("Higher Tier", 0)
        total = foundation + higher
