#!/usr/bin/env python3
"""Top up Algebra mini-subtopics to 60 total by adding Foundation-tier questions only."""
...
# Subtopics where diagrams are strongly preferred.
IMAGE_SUBTOPICS = {
    "algebra|graphs",
}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Top up Algebra subtopics to 60 total using Foundation-only.")
    ...
    for subtopic_id in subtopics:
        if args.subtopic and args.subtopic != subtopic_id:
            continue
        total = fetch_total(supabase_url, headers, subtopic_id)
        if total >= args.target_total:
            print(f"{subtopic_id}: already {total}")
            continue
        missing = args.target_total - total
        print(f"{subtopic_id}: adding {missing} Foundation questions")
        topup_subtopic(
            supabase_url,
            headers,
            subtopic_id,
