-- Make the primary leaderboard cumulative from the sprint start.
-- "sprint" and legacy "month" both use leaderboard_config.effective_start.
...
BEGIN
  ...
  CASE lower(coalesce(p_period, 'sprint'))
    ...
    WHEN 'month' THEN period_start := effective_start;
    WHEN 'sprint' THEN period_start := effective_start;
    ELSE period_start := effective_start;
  END CASE;
