-- Fold cohort + leaderboard placement into get_my_live_mock_attempt_summary so the student
-- dashboard still loads aggregates when standalone cohort/rank RPCs are missing on older DBs.
...
COMMENT ON FUNCTION public.get_my_live_mock_attempt_summary(uuid) IS
  'Returns JSON summary for the caller''s live mock attempt plus cohort placement stats (SECURITY DEFINER).';
