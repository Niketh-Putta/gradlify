-- Freeze the Free Gradlify Premium Mystery Spin leaderboard at the
-- competition close timestamp so later activity no longer changes rankings.
...
DECLARE
  ...
  effective_end timestamptz := '2026-05-06T19:43:03Z'::timestamptz;
  v_user_id uuid;
...
BEGIN
  ...
  WITH correct_totals AS (
    ...
    WHERE ca.created_at >= effective_start
      AND ca.created_at <= effective_end
