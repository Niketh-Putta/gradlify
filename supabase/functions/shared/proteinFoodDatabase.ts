...
const entry = (
  ...
  aliases: string[] = [],
  typicalPortionG?: number,
): FoodDatabaseEntry => ({ id, name, category, proteinPer100g, aliases, typicalPortionG });
...
for (const food of FOOD_DATABASE) {
  ...
  for (const alias of food.aliases) aliasIndex.set(normalizeFoodKey(alias), food);
}
...
export function estimateProtein(name: string, portionGrams?: number): {
...
export function crossValidateItem(
  name: string,
  aiProteinGrams: number,
  portionGrams: number,
  confidence: number,
): { name: string; category: FoodCategory; portionGrams: number; proteinGrams: number; confidence: number } {
  const estimate = estimateProtein(name, portionGrams);
  if (!estimate) {
    return {
      name,
      category: "other",
  ...
  const delta = Math.abs(aiProteinGrams - estimate.proteinGrams);
  const tolerance = Math.max(5, estimate.proteinGrams * 0.35);
  let proteinGrams = aiProteinGrams;
  let adjustedConfidence = confidence;

  if (delta <= tolerance) {
    proteinGrams = Math.round(((aiProteinGrams + estimate.proteinGrams) / 2) * 10) / 10;
    adjustedConfidence = Math.min(1, confidence + 0.1);
  } else {
    proteinGrams = estimate.proteinGrams;
    adjustedConfidence = Math.min(1, confidence + 0.15);
  }
...
export const FOOD_DATABASE_SUMMARY = FOOD_DATABASE.map((f) => ({
  name: f.name,
  category: f.category,
