...
# 4 Batches * 15 Questions = 60 questions per subtopic.
# 60 * 25 subtopics = 1500 strictly equal questions.
...
def generate_batch(topic, index):
    # Using the premium 2.5 Pro endpoint!
        ...
        try:
            with urllib.request.urlopen(req) as response:
                ...
                if direct_insert_supabase(db_rows):
                    print(f"✅ [{topic['sub']}] Generative AI Batch {index}/{BATCHES_PER_SUBTOPIC} inserted successfully.")
                return # exit retry loop
                    
        except urllib.error.HTTPError as e:
            if e.code == 429:
                print(f"⏳ Paid Tier Rate Limit buffer for {topic['sub']} (Batch {index}). Sleeping for 15s...")
                time.sleep(15)
            else:
...
def main():
    print("💎 INITIALIZING PREMIUM PAID-TIER GEMINI-2.5-PRO AI GENERATION...")
    
    total_topics = len(TOPICS)
    for topic_idx, topic in enumerate(TOPICS, 1):
        print(f"\\n--- Pure AI Generation: Topic {topic_idx}/{total_topics}: {topic['sub']} ---")
        for batch_idx in range(1, BATCHES_PER_SUBTOPIC + 1):
            generate_batch(topic, batch_idx)
            # Short buffer between chunks since it's a paid tier with higher limits
            time.sleep(1)
            
    print("\\n\\n🎉 MASSIVE DEPLOYMENT COMPLETED WITH ZERO CLONES!")
